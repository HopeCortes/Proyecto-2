...
def main():
  ...
  # Graficar la curva V-I
  plt.figure(figsize=(10, 5))
  plt.plot(resultados['Voltaje (V)'], resultados['Corriente (A)'], label='Curva V-I')
  plt.scatter(Vmpp, Impp, color='red', label='Punto de Máxima Potencia (MPP)')
  plt.title('Curva Característica V-I del Panel Fotovoltaico')
  plt.xlabel('Voltaje (V)')
  plt.ylabel('Corriente (A)')
  plt.legend()
  plt.grid(True)
  plt.savefig('curva_vi.png')  # Guardar la gráfica como imagen
  plt.close()  # Cerrar la ventana de la gráfica para evitar superposiciones

  # Graficar la curva de potencia
  plt.figure(figsize=(10, 5))
  plt.plot(resultados['Voltaje (V)'], resultados['Potencia (W)'], label='Curva de Potencia')
  plt.scatter(Vmpp, P_max, color='red', label='Punto de Máxima Potencia (MPP)')
  plt.title('Curva de Potencia del Panel Fotovoltaico')
  plt.xlabel('Voltaje (V)')
  plt.ylabel('Potencia (W)')
  plt.legend()
  plt.grid(True)
  plt.savefig('curva_potencia.png')  # Guardar la gráfica como imagen
  plt.close()  # Cerrar la ventana de la gráfica para evitar superposiciones
